...
import { DRACOLoader } from '../../../sumber/three.js/examples/jsm//loaders/DRACOLoader.js';
...
dracoLoader.setDecoderPath( '../../../sumber/three.js/examples/jsm/libs/draco/' );
...
function init() {
    ...
    dracoLoader.load( '../../../sumber/three.js/examples/models/draco/bunny.drc', function ( geometry ) {
    ...
    // folder.add( ssrPass, 'fresnel' ).onChange( () => { groundReflector.fresnel = ssrPass.fresnel } );
    groundReflector.distanceAttenuation = ssrPass.distanceAttenuation = true;
    // folder.add( ssrPass, 'distanceAttenuation' ).onChange( () => { groundReflector.distanceAttenuation = ssrPass.distanceAttenuation } );
    ssrPass.maxDistance = .1;
    groundReflector.maxDistance = ssrPass.maxDistance;
    folder.add( ssrPass, 'maxDistance' ).min( 0 ).max( .5 ).step( .001 ).onChange( ()=>{ groundReflector.maxDistance = ssrPass.maxDistance } );
    // folder.add( params, 'otherMeshes' ).onChange( () => {
    //     if ( params.otherMeshes ) otherMeshes.forEach( mesh => mesh.visible = true );
    //     else otherMeshes.forEach( mesh => mesh.visible = false );
    // } );
    ...
    } ).onChange( function ( value ) { ssrPass.output = value } );
    ssrPass.opacity = 1;
